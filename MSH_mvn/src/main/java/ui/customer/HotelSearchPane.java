...
public class HotelSearchPane {
	...
	private void initPane(){
		...
		Button search=SearchButton.get();
		search.setFont(f);
		pane.add(search,column_index,row_index+2);
		
		Label enter_time=new Label("入住时间");
		enter_time.setFont(f);
		pane.add(enter_time, column_index-2, row_index+3);
		...
		out_time.setFont(f);
		pane.add(out_time,column_index-2,row_index+4);
